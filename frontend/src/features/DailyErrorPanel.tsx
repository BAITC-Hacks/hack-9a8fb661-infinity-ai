...
export function DailyErrorPanel({ daily, turbine, error }: { daily: DailyError[]; turbine: TurbineId; error: string | null }) {
  ...
  return (
    <Panel title="Ошибка по дням · январь" controls={
      ...
      {error ? <Problem>Метрики не загрузились: {error}</Problem> : !data.length ? <Empty>Нет дней с фактом.</Empty> : (
        <div className="h-72">
          <ResponsiveContainer>
            <BarChart data={data} barGap={0} barCategoryGap="0%" margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="mae_base" name="База" fill="#30363d" radius={1} maxBarSize={9} isAnimationActive={false} />
              <Bar dataKey="mae_24" name="Модель 24 ч" fill="#58a6ff" radius={1} maxBarSize={9} isAnimationActive={false} />
              <Bar dataKey="mae_48" name="Модель 48 ч" fill="#f0883e" radius={1} maxBarSize={9} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
