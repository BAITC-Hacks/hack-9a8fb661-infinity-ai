...
export function PowerCurvePanel({ data, error }: { data: PowerCurve | null; error: string | null }) {
  return (
    <Panel title="Кривая мощности" controls={data && <span className="num text-[13px] text-mute">{data.n_hours.toLocaleString('ru')} ч истории</span>}>
      {error ? <Problem>Кривая не загрузилась: {error}</Problem> : !data ? <Empty>Считаю кривую…</Empty> : (
        <div className="h-72">
          <ResponsiveContainer>
            <ComposedChart margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
              ...
              <Line data={data.curve} dataKey="p" stroke={CURVE} strokeWidth={2.5} dot={false} isAnimationActive={false} type="monotone" />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      )}
